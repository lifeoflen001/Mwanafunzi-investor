...
<body class="internal-page">
    ...
    <header class="site-header internal-header" data-header>
        <div class="header-inner container">
            ...
            <nav class="desktop-nav" aria-label="Primary navigation"><a href="{{ route('learn') }}">Learn</a><a href="{{ route('courses') }}">Courses</a><a href="{{ route('tools') }}">Tools</a><a href="{{ route('journal') }}">Journal</a><a href="{{ route('about') }}">About</a></nav>
            <div class="header-actions"><a class="header-contact" href="mailto:{{ $email }}">Contact</a><a class="button button-small button-light" href="{{ route('contact') }}">Start a conversation <span aria-hidden="true">↗</span></a><button class="menu-toggle" type="button" aria-expanded="false" aria-controls="mobile-menu" data-menu-toggle><span class="sr-only">Open menu</span><span></span><span></span></button></div>
        </div>
        <div class="mobile-menu" id="mobile-menu" data-mobile-menu><nav aria-label="Mobile navigation"><a href="{{ route('home') }}">Home <span>01</span></a><a href="{{ route('learn') }}">Learn <span>02</span></a><a href="{{ route('courses') }}">Courses <span>03</span></a><a href="{{ route('tools') }}">Tools <span>04</span></a><a href="{{ route('journal') }}">Journal <span>05</span></a><a href="{{ route('about') }}">About <span>06</span></a></nav><a class="button button-dark" href="{{ route('contact') }}">Contact the desk <span aria-hidden="true">↗</span></a></div>
    </header>
    <main id="main-content">@yield('content')</main>
    <footer class="site-footer"><div class="container"><div class="footer-top"><div class="footer-brand"><a class="brand" href="{{ route('home') }}"><span class="brand-mark" aria-hidden="true"><span></span><span></span><span></span></span><span class="brand-copy"><strong>MWANAFUNZI</strong><small>INVESTOR</small></span></a><p>{!! nl2br(e($footerCopy)) !!}</p></div><div class="footer-nav"><div><span>Explore</span><a href="{{ route('learn') }}">Learn</a><a href="{{ route('courses') }}">Courses</a><a href="{{ route('tools') }}">Tools</a></div><div><span>Company</span><a href="{{ route('journal') }}">Journal</a><a href="{{ route('about') }}">About</a><a href="{{ route('contact') }}">Contact</a></div><div class="footer-contact"><span>Contact</span><a href="mailto:{{ $email }}">{{ $email }}</a><a href="tel:{{ preg_replace('/\D+/', '', $phone) }}">{{ $phone }}</a><p>{{ $location }}</p></div></div></div><div class="footer-bottom"><span>© {{ date('Y') }} {{ $brandName }}</span><div><a href="{{ route('legal', 'privacy-policy') }}">Privacy</a><a href="{{ route('legal', 'terms') }}">Terms</a><a href="{{ route('legal', 'risk-disclosure') }}">Risk disclosure</a></div><strong>ALWAYS A MWANAFUNZI.</strong></div><p class="site-disclaimer">{{ $disclaimer }}</p></div></footer>
</body>
</html>
