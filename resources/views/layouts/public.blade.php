@php
    ...
    $headerNavigation = \App\Models\NavigationItem::visible()->where('location', 'header')->whereNull('parent_id')->get();
    $footerNavigation = \App\Models\NavigationItem::visible()->where('location', 'footer')->whereNull('parent_id')->orderBy('menu_group')->get()->groupBy('menu_group');
@endphp
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <header class="site-header internal-header" data-header>
        <div class="header-inner container">
            ...
            <nav class="desktop-nav" aria-label="Primary navigation">
                @forelse($headerNavigation as $item)<a class="{{ $item->route_name && request()->routeIs($item->route_name) ? 'active' : '' }}" href="{{ $item->href() }}" target="{{ $item->target }}" @if($item->route_name && request()->routeIs($item->route_name)) aria-current="page" @endif>{{ $item->label }}</a>@empty @foreach($businessUnits as $businessUnit)<a class="{{ request()->routeIs($businessUnit->route_name) ? 'active' : '' }}" href="{{ route($businessUnit->route_name) }}">{{ $businessUnit->name }}</a>@endforeach<a href="{{ route('about') }}">About</a>@endforelse
            </nav>
